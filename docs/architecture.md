...
- `mars-bluetooth-hci` (v0.13.0) — the encoder, parser, and serialize-only C FFI for Bluetooth HCI Channel Sounding subevent-result events. Built as both a `lib` (Rust API) and a `staticlib` (`libmars_bluetooth_hci.a`, which is what the firmware links).
