...
- `mars-bluetooth-hci` (v0.13.2) — the encoder, parser, and serialize-only C FFI for Bluetooth HCI Channel Sounding subevent-result events. Built as both a `lib` (Rust API) and a `staticlib` (`libmars_bluetooth_hci.a`, which is what the firmware links).
...
The production consumer is `mars-cs-nrf54l` (open nRF54L firmware), which builds the event struct in C and serializes it for UART. The decoder is `mars-ranging-demo` (a public repo with a closed-source GUI), which receives the UART stream and deserializes it. This repo is the **encode side**; the decode side is closed-source, so this repo — not `mars-ranging-demo` — is the authoritative source for the wire-format contract (ADR-0001). That encode-only boundary belongs to the UART wire path; persisted-frame storage replay is a separate, versioned contract with an in-repo decoder ([ADR-0003](adr/0003-versioned-persisted-frame-codec.md); see §Persisted-frame replay below).
...
| Symbol | Role | Source |
|---|---|---|
| `serialize_subevent_result_event` | Serialize a `&SubeventResultEvent` into a `SerializedData` buffer, optionally COBS-encoded. | `mars-bluetooth-hci/src/libc.rs:73` |
| `serialize_log_message` | Serialize a C-string log message into a `SerializedData` buffer, optionally COBS-encoded. | `mars-bluetooth-hci/src/libc.rs:92` |
...
This is the decode-side path, available only to Rust consumers. `impl TryFrom<&[u8]> for SubeventResultEvent` (`subevent_result.rs:976`) parses raw HCI subevent bytes: it handles subevent codes `0x31` (`CS_CONFIG_COMPLETE`) and `0x32` (`CS_SUBEVENT_RESULT_CONTINUE`), decodes the header fields, and `push_steps` (`subevent_result.rs:688-831`) walks the per-step byte layout. It is consumed by the file-reader helper `read_file` (`mars-bluetooth-hci/src/event/hci_le_cs/hci_file_reader.rs:52`) and by tests.
...
2. It is wrapped in `SerializableRef::SubeventResultEvent(p_event)` — a borrowing enum so the ~16 KB struct is not cloned or heap-allocated. `SerializableRef` produces an identical wire format to the owned `Serializable` enum (asserted by the `log_message_wire_format_matches` test).
3. It is serialized with `postcard::to_allocvec_cobs` (the `use_cobs=true` path: serde + COBS byte-stuffing + a trailing `0x00` sentinel fused into one call, so the stream is self-framing with no out-of-band framing) or plain postcard (the `use_cobs=false` path: no COBS, no `0x00` — a deliberate unframed variant for non-streaming/recording use; the subevent-result envelope serializes into a pre-sized buffer through the shared core, see ADR-0003; see also ADR-0001).
...
The envelope on the wire is the `SerializableRef`/`Serializable` enum (`SubeventResultEvent` or `LogMessage` variant) — the receiver deserializes the enum and dispatches accordingly. There is **no in-band version or magic byte** in the envelope today; versioning is currently by git-tag co-pinning of the library with paired firmware/evaluation-app releases. Whether to add an in-band version/magic byte is an open decision (#15; see [wire-format.md](wire-format.md) §Versioning and compatibility). That no-in-band-version statement describes the UART envelope; the persisted-frame storage contract keeps the same envelope bytes but versions them out-of-band via its frame descriptor ([ADR-0003](adr/0003-versioned-persisted-frame-codec.md)) — see §Persisted-frame replay below.
...
### Persisted-frame replay (Rust API only)

Decoded subevent-result events can also be stored for later replay in MARS storage through the versioned persisted-frame codec, `event::hci_le_cs::persisted_frame` (`mars-bluetooth-hci/src/event/hci_le_cs/persisted_frame.rs`) — available when the `persisted-frame` feature is enabled (part of the default build) and deliberately unavailable to the embedded no-std feature set. This is a separate contract from the UART wire path: `encode` validates the event's counts and emits bare frame bytes with no in-band version, the caller persists the descriptor returned by `current_frame_descriptor()` alongside them, and `decode` dispatches only on that declared descriptor — strict, so padded, over-written, or concatenated records are rejected with a trailing-bytes error, a truncated record fails decode inside postcard, a leading byte that is not a V1 envelope tag is rejected as a schema mismatch or corruption, a well-formed log-message envelope is rejected as the wrong value kind (a corrupt log record fails decode), and an event breaking the parser's step or antenna-path bounds is rejected as invalid; nothing is silently decoded. V1 frames are byte-compatible with the FFI serializer's `use_cobs=false` path (both call the same shared serializer core), which costs a fixed ~13.3 KB per frame regardless of `step_count` — the price of byte-compatibility; shrinking frames requires a new declared version. Like the Path B parser, the codec never crosses the C FFI (ADR-0002): it is a Rust-API-only concern. The decision, retention, and migration policy are recorded in [ADR-0003](adr/0003-versioned-persisted-frame-codec.md); the versioning statements live in [wire-format.md](wire-format.md) §Versioning and compatibility.
...
- **Identity fields are caller-set by design.** The parser (`impl TryFrom<&[u8]> for SubeventResultEvent`, `subevent_result.rs:976`) populates every field it can decode from the HCI bytes but leaves `local_mac` and `peer_mac` at `0` — the raw subevent bytes do not carry node identity; the caller fills them from out-of-band context (in Path A the firmware sets all three directly in C before calling `serialize_subevent_result_event`). `origin` is different: Mode 1 and Mode 3 steps need the node's role *while parsing*, so the caller passes it up front via `try_from_with_origin` (the file-reader helper `read_file`, `hci_file_reader.rs:52`, takes it from the vendor format's `requester`/`reflector` label); parsing the same bytes through plain `TryFrom` (origin `Unknown`) fails with `UnknownOriginForMode` for such captures. This is intentional, not a parser bug.
- **Invalid (0xFF) step slots are preserved as sentinel steps.** Step_Mode `0xFF` is a MARS sentinel for a step slot reported without valid data — the Bluetooth spec marks all non-0..3 modes Reserved and signals an aborted step by zero length, which the parser also accepts as a sentinel slot. The parser preserves such slots as steps carrying the `ModeRoleSpecificInfoKind::Invalid` kind (defaulted payload fields) instead of collapsing them, so `steps[..step_count]` maps 1:1 to the reported slots and consumers filter on the sentinel mode. Parse output for captures containing such slots changed with this release: earlier builds left a fabricated default step behind and shifted the following steps to lower indices — and on the UART wire path such captures now carry the appended `Invalid` tag (11), which decoders built against the 0.13.2 enum reject; that is governed by the UART path's tag-co-pinning/lockstep-release policy (wire-format.md §Versioning and compatibility). Note the sentinel's cost as a corruption channel: a step header corrupted from a real mode to `0xFF` silently absorbs its declared payload bytes into the sentinel, while corruption to any other reserved mode is rejected with `InvalidModeType`.
- **Packet-level sentinels pass through as raw values.** The parser honors the spec's not-available sentinels for the fields it converts (`Mode0Data::to_ppm` and `RoundTripTimeRoleTiming::to_seconds` return `None`), but the packet-level fields decode unconverted: a packet phase correction term of `0xFFFFFFFF`, `Packet_RSSI` of `0x7F`, and NADM `0xFF` ("unknown") decode as ordinary field values, and reserved `Frequency_Compensation` encodings (0x8000–0xBFFF, 0xC001–0xFFFF) decode as plausible ppm values — filtering those is left to consumers.
...
- [adr/0003-versioned-persisted-frame-codec.md](adr/0003-versioned-persisted-frame-codec.md) — the versioned persisted-frame storage-replay codec decision: out-of-band `FrameDescriptor` versioning, V1 byte-compatibility with the non-COBS FFI representation, and the retention/migration policy.
