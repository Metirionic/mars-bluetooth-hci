...
Decoded subevent-result events can also be stored for later replay in MARS storage through the versioned persisted-frame codec, `event::hci_le_cs::persisted_frame` (`mars-bluetooth-hci/src/event/hci_le_cs/persisted_frame.rs`) — available in the default host configuration (`std`+`alloc`+`libc`) and deliberately unavailable to the embedded no-std feature set. This is a separate contract from the UART wire path: `encode` emits bare frame bytes with no in-band version, the caller persists the descriptor returned by `current_frame_descriptor()` alongside them, and `decode` dispatches only on that declared descriptor — strict, so padded, over-written, or concatenated records are rejected with a trailing-bytes error, a truncated record fails decode inside postcard, a log-message envelope is rejected as the wrong value kind, and an event breaking the parser's step or antenna-path bounds is rejected as invalid; nothing is silently decoded. V1 frames are byte-compatible with the FFI serializer's `use_cobs=false` path (both call the same shared serializer core), which costs a fixed ~13.3 KB per frame regardless of `step_count` — the price of byte-compatibility; shrinking frames requires a new declared version. Like the Path B parser, the codec never crosses the C FFI (ADR-0002): it is a Rust-API-only concern. The decision, retention, and migration policy are recorded in [ADR-0003](adr/0003-versioned-persisted-frame-codec.md); the versioning statements live in [wire-format.md](wire-format.md) §Versioning and compatibility.
